...
const exeFile = path.join(s.dirCompile(), 'mssqltask-app.exe')
...
fs.writeFileSync(path.join(s.dirCompile(), 'readme.txt'), readme.join('\n'),'utf8')

// @ts-ignore
var zip = new AdmZip()
zip.addLocalFile(exeFile)
zip.addLocalFile(path.join(s.dirCompile(), 'readme.txt'))
zip.writeZip(path.join(s.dirCompile(), `mssqltask-app-win64-${pj.version.replace(/\./g, '-')}.zip`))
